class ResponseFormatter {
            ...
            quick_actions: [
                {
                    ...
                    data: { movie_id: movie._id, movie_title: movie.title },
                },
                {
                    text: 'Đặt vé phim này',
                    action: 'booking_flow',
                    data: { movie_id: movie._id, movie_title: movie.title },
                },
    ...
    formatSmartMissingQuestion(missingParam, context = {}) {
        ...
        const message =
            templates[Math.floor(Math.random() * templates.length)] || `Bạn vui lòng cung cấp ${missingParam}`;
    ...
    formatErrorResponse(errorType = 'api_error') {
        const errorTemplates = {
            ...
            branch_not_found: {
                message:
                    '😕 Rất tiếc, tôi không tìm thấy rạp nào ở địa điểm đó. Bạn có muốn xem danh sách các rạp đang hoạt động không?',
                suggestions: [{ text: 'Tất cả các rạp', action: 'list_branches' }],
            },
            schedule_not_found: {
    ...
    generateContextualQuickActions(interactionContext = null, responseType = '', currentData = {}) {
        ...
        if (
            interactionContext.frequentBranches?.length > 0 &&
    ...
    formatMovieListWithContext(movies, query = {}, interactionContext = null) {
        ...
        const contextualSuggestions = this.generateContextualQuickActions(interactionContext, 'movie_list', {
            movies,
            query,
        });
            ...
            let quickActions = [
                {
                    text: 'Xem lịch chiếu',
                    action: 'find_schedules',
                    data: { movie_title: movie.title, movie_id: movie._id },
                },
                { text: 'Xem chi tiết', action: 'movie_details', data: { movie_id: movie._id } },
            ];
    ...
    formatSearchConversationWithContext(interactionContext = null) {
        ...
        const defaultSuggestions = this.generateContextualQuickActions(interactionContext, 'search_conversation');
        ...
        try {
            switch (analysis.intent) {
                ...
                case 'search_for_schedule': {
                    if (!retrievedData) return this.formatErrorResponse('movie_not_found');
                    return this.formatMovieListForScheduleWithContext(
                        retrievedData,
                        analysis.entities,
                        interactionContext,
                    );
    ...
    formatScheduleResponseWithContext(scheduleData, entities = {}, interactionContext = {}) {
        ...
        if (
            interactionContext.lastInteractedMovieTitle &&
