...
function applyPostProcessingRules(result, userQuery, conversationContext) {
  ...
  const intentMappings = { 'find_movies': lowerQuery.includes('sắp chiếu') ? 'get_upcoming' : 'get_now_showing', 'get_movies': lowerQuery.includes('sắp chiếu') ? 'get_upcoming' : 'get_now_showing' };
...
function normalizeLocation(location) {
  ...
  if (lowerLocation.includes('nguyễn văn cừ') || lowerLocation.includes('chi nhánh chính') || lowerLocation.includes('rạp chính')) return 'Nguyễn Văn Cừ';
  if (lowerLocation.includes('nguyễn huệ') || lowerLocation.includes('trung tâm') || lowerLocation.includes('downtown') || lowerLocation.includes('quận 1')) return 'Nguyễn Huệ';
  if (lowerLocation.includes('huỳnh tấn phát') || lowerLocation.includes('quận 7') || lowerLocation.includes('q7') || lowerLocation.includes('phú mỹ hưng')) return 'Huỳnh Tấn Phát';
  return location;
}
...
function performFallbackAnalysis(userQuery, conversationContext) {
  const lowerQuery = userQuery.toLowerCase().trim();
  const result = { intent: 'greeting', entities: {}, context: { needs_followup: false }, confidence: 0.2, fallback: true };
  ...
  if (conversationContext && conversationContext.lastIntent === 'find_schedules' && conversationContext.missingParams) {
    result.intent = 'find_schedules';
      ...
      case 'find_schedules': {
        try {
          ...
          if (!branchId && entities.location) {
            const branch = await Branch.findOne({ name: { $regex: new RegExp(entities.location, 'i') } }).lean();
            if (branch) {
              branchId = branch._id;
            }
          }
