...
class CacheManager {
    ...
    static async invalidateScheduleCache(scheduleId) {
        try {
            const keys = [`seat_map_${scheduleId}`, `schedule_cache_${scheduleId}`];

            await Promise.all(
                keys.map(key =>
                    redisClient.del(key).catch(err => console.warn(`Failed to delete cache key ${key}:`, err)),
                ),
            );
        } catch (error) {
    ...
    static async cleanupCache() {
        try {
            ...
            const patterns = ['seat_layout_*', 'branch_info_*', 'seat_map_*', 'schedule_cache_*'];
