...
class ScheduleRetrieverService {
        ...
        try {
            ...
            const pipeline = [
                    ...
                    $addFields: {
                        totalSeats: {
                            $ifNull: [
                                ...
                                { $multiply: ['$screenData.size.rows', '$screenData.size.columns'] },
                            ],
                        },
                ...
                },
                {
                    $addFields: {
                        availableSeats: {
                            $subtract: ['$totalSeats', { $add: ['$occupiedSeatsCount', '$heldSeatsCount'] }],
                        },
                    },
                },
                {
                    $project: {
                        ...
                        availableSeats: 1,
                        dateFormatted: {
                            $dateToString: { format: '%Y-%m-%d', date: '$startTime', timezone: 'Asia/Ho_Chi_Minh' },
                        },
                        timeFormatted: {
                            $dateToString: { format: '%H:%M', date: '$startTime', timezone: 'Asia/Ho_Chi_Minh' },
                        },
                    },
                },
    ...
    async getSchedulesByFilters(filters = {}) {
        const { branchId, movieId, date, startDate, endDate, limit = 20, includeExpired = false } = filters;
        try {
            const pipeline = [
                { $lookup: { from: 'movies', localField: 'movie', foreignField: '_id', as: 'movieData' } },
                { $lookup: { from: 'screens', localField: 'screen', foreignField: '_id', as: 'screenData' } },
