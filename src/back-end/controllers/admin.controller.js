...
const createUser = async (req, res) => {
    try {
        ...
        const needsBranch = !(
            (rolesArray.includes('customer') && rolesArray.length === 1) ||
...
const getAllProfiles = async (req, res) => {
    try {
        ...
        const users = await User.find()
            ...
            .select(
                '-password -wishlist -watchHistory -lastAccess -lastOrder -passwordResetToken -passwordResetExpires',
            );
...
const getDetailedProfile = async (req, res) => {
    try {
        ...
        const user = await User.findById(req.params.userId).select(
            '-wishlist -watchHistory -lastAccess -lastOrder -passwordResetToken -passwordResetExpires',
        );
        if (!user) {
...
const updateUserDetails = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select(
            '-roles -wishlist -watchHistory -lastAccess -isLocked -lastOrder -passwordResetToken -passwordResetExpires',
        );
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
        ...
        if (updateData.branch) {
            ...
            // Only users with ONLY customer role OR ONLY administrator role don't need branch
            const needsBranch = !(
                (currentRoles.includes('customer') && currentRoles.length === 1) ||
        ...
        await Promise.all([redisClient.del('userList'), redisClient.del(`user:${userId}`)]);
...
const updateUserRoles = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select(
            '-name -email -phone -birthday -gender -branch -password -wishlist -watchHistory -isLocked -lastAccess -lastOrder -passwordResetToken -passwordResetExpires',
        );
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
...
const updateUserStatus = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select(
            '-name -email -phone -birthday -gender -branch -password -wishlist -watchHistory -roles -lastAccess -lastOrder -passwordResetToken -passwordResetExpires',
        );
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
...
const updatePromotion = async (req, res) => {
    try {
        ...
        if (updateData.appliedLoyaltyRank !== undefined) {
            ...
            if (
                (typeof updateData.appliedLoyaltyRank === 'string' && updateData.appliedLoyaltyRank.trim() === '') ||
                updateData.appliedLoyaltyRank === 'None'
            ) {
        ...
        const promotion = await Promotion.findOneAndUpdate({ promotionCode: promotionCode.toUpperCase() }, updateData, {
            new: true,
            runValidators: true,
        });
        if (!promotion) {
...
const getPublicPromotions = async (req, res) => {
    try {
        ...
        let bannerCondition = {
            $and: [{ bannerImage: { $ne: null } }, { bannerImage: { $ne: '' } }, { bannerImage: { $exists: true } }],
        };
        ...
        if (!userId) {
            ...
            loyaltyFilter = {
                $or: [{ appliedLoyaltyRank: null }, { appliedLoyaltyRank: '' }],
            };
        ...
        const finalQuery = {
            $and: [promotionQuery, loyaltyFilter],
        };
...
const updateBranch = async (req, res) => {
    try {
        ...
        // Update branch
        const updatedBranch = await Branch.findByIdAndUpdate(branchId, updateData, { new: true, runValidators: true });
...
const getAllBranches = async (req, res) => {
    try {
        ...
        const branches = await Branch.find({}).select(
            '_id name address city imageURL location isActive createdAt updatedAt',
        );
