...
const holdSeats = async (req, res) => {
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            const schedule = await Schedule.findById(scheduleId).populate('screen', 'size').session(session).lean();
            ...
            if (existingHolds.length > 0) {
                if (replaceExisting) {
                    ...
                    const ownedHolds = existingHolds.filter(
                        hold =>
                            (userId && hold.user?.toString() === userId) || (sessionId && hold.sessionId === sessionId),
                    );
                    ...
                    await SeatHold.deleteMany({
                        schedule: scheduleId,
                        seatNumber: { $in: uniqueSeatNumbers },
                        $or: [...(userId ? [{ user: userId }] : []), ...(sessionId ? [{ sessionId: sessionId }] : [])],
                    }).session(session);
                } else {
...
const applyDiscounts = async ({ user, promotionCode, snackTotal, movieTotal, session = null }) => {
    ...
    if (promotionCode) {
        ...
        if (
            snackTotal &&
            (promo.appliedProduct === 'Snack' || promo.appliedProduct === 'All') &&
            snackTotal >= promo.minimumSpend
        ) {
        ...
        if (
            movieTotal &&
            (promo.appliedProduct === 'Movie' || promo.appliedProduct === 'All') &&
            movieTotal >= promo.minimumSpend
        ) {
...
const createTicket = async (req, res) => {
    ...
    try {
        const { noLoginCustomerInfo, branch, promotionCode, movieTicket, snackTicket } = req.body;
        ...
        await session.withTransaction(async () => {
            ...
            if (movieTicket) {
                ...
                await SeatHold.deleteMany({
                    schedule: schedule,
                    seatNumber: { $in: seats },
                    $or: [{ user: customer }, { sessionId: req.sessionID }],
                }).session(session);
            }
            ...
            if (snackTicket) {
                ...
                const { validatedSnackList } = await calculateTotalAndUpdateStock(snackList, branchData._id, session);
                // Use total from API (required)
...
const deleteTicket = async (req, res) => {
    try {
        ...
        if (isMovie && ticket.seats && ticket.seats.length > 0) {
            await Schedule.findByIdAndUpdate(ticket.schedule, { $pullAll: { OccupiedSeat: ticket.seats } });
