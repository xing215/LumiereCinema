    ...
    try {
        ...
        if (updateData.discountedPrice !== undefined) {
            ...
            if (
                ...
                updateData.discountedPrice > priceToCompare
            ) {
                return res.status(400).json({
...
const getSnackList = async (req, res) => {
    try {
        ...
        const snacks = await Snack.find({ branch: branchId }).select(
            'name shortname price discountedPrice imageURL stock isHidden description createdAt updatedAt',
        );
                    ...
                    pipeline: [
                        ...
                        },
                        {
                            $lookup: {
                                ...
                                localField: 'movie',
                                foreignField: '_id',
                                pipeline: [{ $match: { isHidden: false, releaseDate: { $lte: new Date() } } }],
                                as: 'movieData',
                            },
                        },
...
const editMovieSchedule = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(branchId) || !mongoose.Types.ObjectId.isValid(scheduleId)) {
            return res.status(400).json({
                message: 'Invalid ID format.',
            });
        }
        ...
        const existingSchedule = await Schedule.findById(scheduleId).populate('screen', 'branch');
...
const getSchedulesByBranchLogic = async (req, res, branchId, movieId, user, accessLevel) => {
    try {
        ...
        const timeMatch =
            accessLevel === 'cashier' || accessLevel === 'administrator' || accessLevel === 'branchmanager'
                ? { endTime: { $gt: now } }
                : { startTime: { $gt: now } };
...
const getScreenById = async (req, res) => {
    try {
        ...
        // 3. Validate ObjectId formats
        if (!mongoose.Types.ObjectId.isValid(branchId) || !mongoose.Types.ObjectId.isValid(screenId)) {
            return res.status(400).json({
                message: 'Invalid ID format.',
            });
        }
...
const updateScreen = async (req, res) => {
    try {
        ...
        const updatedScreen = await Screen.findByIdAndUpdate(screenId, updateData, { new: true, runValidators: true });
...
const deleteScreen = async (req, res) => {
    try {
        ...
        if (schedulesCount > 0) {
            ...
            const deactivatedScreen = await Screen.findByIdAndUpdate(screenId, { isActive: false }, { new: true });
...
const getScreenSeats = async (req, res) => {
    try {
        ...
        // 6. Get seats
        const Seat = require('../models/Seat');
        const seats = await Seat.find(filterConditions).sort({ 'location.row': 1, 'location.column': 1 });
...
const createSeat = async (req, res) => {
    try {
        ...
        if (
            rowNumber < 1 ||
            rowNumber > screen.size.rows ||
            location.column < 1 ||
            location.column > screen.size.columns
        ) {
            return res.status(400).json({
    ...
    try {
        ...
        for (let i = 0; i < seats.length; i++) {
            ...
            if (
                rowNumber < 1 ||
                rowNumber > screen.size.rows ||
                seat.location.column < 1 ||
                seat.location.column > screen.size.columns
            ) {
                errors.push(
                    `Seat ${i + 1}: Location out of screen bounds (row: ${seat.location.row}, column: ${seat.location.column}, screen: ${screen.size.rows}x${screen.size.columns})`,
                );
                continue;
            }
...
const updateSeat = async (req, res) => {
    try {
        ...
        // 7. Update the seat
        const updatedSeat = await Seat.findByIdAndUpdate(seatId, updateData, { new: true, runValidators: true });
...
const deleteSeat = async (req, res) => {
    try {
        ...
        if (scheduleCount > 0) {
            ...
            const hiddenSeat = await Seat.findByIdAndUpdate(seatId, { isHidden: true }, { new: true });
