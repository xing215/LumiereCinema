...
const ErrorModal = ({ errorCode = null, errorMsg = "An error occurs" }) => {
  const [visible, setVisible] = useState(true);
  if (!visible) return null;

  const fullMsg = errorCode ? `${errorCode}: ${errorMsg}` : errorMsg;
  return (
    <div className="max-w-full w-[calc(100vw-3rem)] px-6 py-4 h-16 fixed top-5 left-1/2 -translate-x-1/2 z-[9999] flex items-center rounded-xl overflow-hidden">
