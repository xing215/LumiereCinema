...
const StaffLayout = ({ 
    ...
    return (
        <div className={`flex h-screen w-screen ${backgroundClass}`}>
            ...
            {/* Main Content */}
            <div className={`flex-1 transition-all duration-300 ${isCollapsed ? 'ml-0 lg:ml-16' : 'ml-0 lg:ml-64'} relative overflow-hidden ${className} ${isMobileMenuOpen ? 'blur-sm lg:blur-none' : ''}`}>
                {children}
            </div>
        </div>
    );
