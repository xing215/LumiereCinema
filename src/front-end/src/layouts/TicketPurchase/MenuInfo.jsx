...
const MenuInfo = ({ onNext, onBack, movieTicketData, snackTicketData, updateMovieTicket, updateSnackTicket }) => {
    ...
    const handleNext = () => {
        if (canProceed) {
            ...
            showInfo('Information Required', 'Please fill in all required customer information.');
        }
    };
        ...
        <div className="relative flex w-screen items-center justify-center pt-3 md:pt-7">
            <div className="relative flex h-full min-h-auto w-full flex-row justify-start rounded-xl md:min-h-[470px] md:w-screen lg:h-auto lg:w-[75vw]">
                ...
                <div className="relative flex min-w-[50%] flex-1 flex-col items-center justify-between">
                    ...
                    <div className="block h-auto pt-5 md:hidden">
                        <TicketDetail movieTicketData={movieTicketData} snackTicketData={snackTicketData} isStaff={false} />
                    </div>
                    ...
                    <div className="relative flex flex-col items-center justify-start gap-4">
                        ...
                        <InputField label="Name" name="name" value={customerInfo.name} onChange={(e) => handleInputChange('name', e.target.value)} />

                        <InputField label="Email" name="email" type="email" value={customerInfo.email} onChange={(e) => handleInputChange('email', e.target.value)} errors={errors.email} />

                        <InputField label="Phone Number" name="phone" value={customerInfo.phone} onChange={(e) => handleInputChange('phone', e.target.value)} errors={errors.phone} />
                    </div>
                    ...
                    <div className="flex w-[80vw] flex-row items-center justify-center gap-2 px-4 pt-8 pb-10.5 sm:px-8 md:w-[35vw] md:px-10 md:pb-6 lg:w-[30vw] lg:px-12">
                        ...
                        <NextNaviButton text="PAYMENT" onClick={handleNext} showTextOnMobile={true} disabled={!canProceed} />
                        ...
                        <div className="absolute bottom-0 justify-start pb-4 text-center md:pb-0.5">
                            ...
                            <span
                                className="cursor-pointer font-['Libre_Franklin'] text-[13px] font-bold text-white hover:underline md:text-[12px] lg:text-[13px]"
