...
const Buttons = ({ text, onClick, loading }) => {
    return (
        <button className="relative flex flex-row cursor-pointer h-auto w-full z-10 rounded-2xl py-3 shadow-[inset_0px_0px_50px_3px_rgba(155,47,255,1.00)] justify-center items-center bg-pink-400 p-2 text-white hover:bg-purple-700" 
        onClick={loading ? null : onClick}>
            <span className="relative text-sm md:text-md font-['Unbounded'] text-center h-auto w-[80%] font-semibold">{text}</span>
        </button>
    );
};
...
if (formData.gender.toLowerCase() !== (profile?.gender || '').toLowerCase()) UpdateData.gender = formData.gender.toLowerCase();
    ...
    return (
        <div className="flex flex-col gap-3 justify-start items-start w-full px-4sm:px-0 max-w-[690px]">
            ...
            {updateError && (
                    <div className="rounded-md bg-red-100 p-3 w-full text-center">
                        <p className="font-['Libre_Franklin'] text-md text-red-800">{updateError}</p>
                    </div>
                )}
            {/* Registration Form */}
            <form onSubmit={handleSubmit} className="w-full space-y-4 sm:space-y-5 md:space-y-6">
                <div>
                    ...
                    <input
                        ...
                        className={`bg-opacity-70 h-10 w-full  disabled:bg-zinc-300/5 disabled:text-white disabled:ring-1 disabled:ring-amber-50 rounded-lg bg-zinc-300 px-3 text-black placeholder-gray-600 focus:ring-2 focus:outline-none sm:h-11 sm:px-4 md:h-12 lg:h-13 xl:h-14 ${errors.name ? 'ring-2 ring-red-500 focus:ring-red-500' : 'focus:ring-purple-500'} focus:bg-opacity-90 font-['Unbounded'] text-sm sm:text-base md:text-lg`}
                        required
                        disabled={canEdit}
                    />
                ...
                <div className="flex flex-col gap-4 md:flex-row md:gap-4">
                    <div className="flex-1">
                        ...
                        <input
                            ...
                            onChange={handleInputChange}
                            className={`bg-opacity-70 h-10 disabled:bg-zinc-300/5 disabled:text-white disabled:ring-1 disabled:ring-amber-50 w-full rounded-lg bg-zinc-300 px-3 text-black focus:ring-2 focus:outline-none sm:h-11 sm:px-4 md:h-12 lg:h-13 xl:h-14 ${errors.birthday ? 'ring-2 ring-red-500 focus:ring-red-500' : 'focus:ring-purple-500'} focus:bg-opacity-90 font-['Unbounded'] text-sm sm:text-base md:text-lg`}
                            required
                            disabled={canEdit}
                        />
                ...
                >
                    ...
                    <div
                        className={`
                            absolute top-0 left-1/2 z-50 w-max -translate-x-1/2 rounded
                            bg-indigo-100 backdrop-blur-[10px] font-['Libre_Franklin'] px-3 py-1
            text-xs text-black shadow-lg
            transition-all duration-200
            ${hovered && !canEdit ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}
        `}
    >
        Please contact support to change your email
                    ...
                    <input
                        ...
                        onChange={handleInputChange}
                        className={`bg-opacity-70 h-10 w-full disabled:bg-zinc-300/5 disabled:text-white disabled:ring-1 disabled:ring-amber-50 rounded-lg bg-zinc-300 px-3 text-black placeholder-gray-600 focus:ring-2 focus:outline-none sm:h-11 sm:px-4 md:h-12 lg:h-13 xl:h-14 ${errors.email ? 'ring-2 ring-red-500 focus:ring-red-500' : 'focus:ring-purple-500'} focus:bg-opacity-90 font-['Unbounded'] text-sm sm:text-base md:text-lg`}
                        required
                        disabled={true}
                    />
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={handleInputChange}
                        className={`bg-opacity-70 h-10 disabled:bg-zinc-300/5 disabled:text-white disabled:ring-1 disabled:ring-amber-50 w-full rounded-lg bg-zinc-300 px-3 text-black placeholder-gray-600 focus:ring-2 focus:outline-none sm:h-11 sm:px-4 md:h-12 lg:h-13 xl:h-14 ${errors.phone ? 'ring-2 ring-red-500 focus:ring-red-500' : 'focus:ring-purple-500'} focus:bg-opacity-90 font-['Unbounded'] text-sm sm:text-base md:text-lg`}
                        required
                        disabled={canEdit}
            ...
            <div className='md:w-[62%] w-full pt-4 flex flex-col gap-3 max-w-[350px] justify-center mx-auto md:mx-0'>
                {!canEdit ? (
