        ...
        <div className="flex flex-col items-center justify-center w-full h-full overflow-hidden">
            <div className="flex items-start justify-center h-[80vh] rounded-xl overflow-hidden w-[90%] relative">
                ...
                <div className="flex flex-col gap-6 items-center w-full max-h-full py-4 px-6">
                    {/* Date selector */}
                    {loading ? <div className="md:text-md h-auto items-center justify-center  font-['Unbounded'] text-base font-black text-white mx-2"> • • •</div> : <div className="flex flex-row gap-2 mb-4 justify-center items-center flex-wrap w-full">
                        {uniqueDates.map(dateStr => (
                            <button
                                key={dateStr}
                                className={`rounded-lg text-2xl px-6 py-5 text-center font-['Unbounded'] text-white mix-blend-color-dodge font-semibold bg-zinc-300/60 ${selectedDate === dateStr ? 'ring-2 ring-zinc-500' : ''}`}
                    ...
                    {/* Schedule buttons */}
                    {loading ? <div className="md:text-md h-auto items-center justify-center  font-['Unbounded'] text-base font-black text-white mx-2"> • • •</div> : <div className="flex flex-row flex-wrap gap-4 items-start justify-center w-full">
                        {filteredSchedules.length > 0 ? (
                            ...
                            <div className="text-gray-500 font-semibold">No showtimes for this date.</div>
                        )}
                    </div>}
                </div>
            </div>
