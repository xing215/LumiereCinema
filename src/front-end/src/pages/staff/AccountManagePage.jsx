...
const AccountManagePage = () => {
    ...
    const [accountRows, setAccountRows] = useState(
        Array.from({ length: 10 }, () => ['TickButton', '001', 'Nguyễn Thiên Nhã Ra', 'TinaLe','091666666','Edit'])
    );
        ...
        <StaffLayout backgroundClass="bg-zinc-300/70">
            <MobileNotSupported>
                ...
                {tickedAccounts.size > 0 ? (
                    <DeleteButton onClicked={() => setShowConfirmDeleteAccount(true)} />
                ) : (
                    <AddButton />
                )}
                {showConfirmDeleteAccount && (
                    <ConfirmationModal
                        item={tickedAccounts.size}
                ...
                {editedAccountIndex !== null && (
                    <EditAccountInformationModal
                        onClose={() => setEditedAccountIndex(null)}
                        handleConfirm={() => setEditedAccountIndex(null)}
                    />
                )}
                <ManageTable
                    data={accountRows}
