...
const BranchManagePage = () => {
    ...
    const [branchRows, setBranchRows] = useState(
        Array.from({ length: 10 }, () => ['TickButton', '1', 'Cao Thắng', '123 Nguyễn Văn Cừ, Phường Chợ Quán', 'ActiveButton'])
    );
    ...
    return (
        <StaffLayout backgroundClass="bg-zinc-300/70">
            <MobileNotSupported>
                <SearchButton />
                {tickedBranches.size > 0 ? (
                    <DeleteButton onClicked={() => setShowConfirmDeleteBranch(true)} />
                ) : (
                    <AddButton />
                )}
