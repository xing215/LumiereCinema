...
const SnackManagePage = () => {
    ...
    return (
        <StaffLayout backgroundClass="bg-zinc-300/70">
            <MobileNotSupported>
                <SearchButton onSearch={handleSearch} placeholder="Search snacks by name..." />

                {isAddingSnack ? (
                    <AddSnackButtons onConfirm={handleConfirmAddSnack} onCancel={handleCancelAddSnack} isLoading={updateLoading} />
                ) : tickedSnacks.size > 0 ? (
                    <div className="absolute right-1/12 z-10 flex items-end gap-4 lg:top-1/7 xl:top-[10vh]">
                        <DeleteButton onClicked={handleDeleteClick} disabled={deleteLoading} />
                    </div>
                ) : (
                    <div className="absolute right-1/12 z-10 flex items-end gap-4 lg:top-1/7 xl:top-[10vh]">
                        <AddButton text="Add Snack" onClick={handleStartAddSnack} disabled={loading || updateLoading} />
                    </div>
                )}

                {loading ? (
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="font-['Unbounded'] text-2xl text-black">Loading snacks...</div>
                    </div>
                ) : error ? (
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="font-['Unbounded'] text-2xl text-red-600">Error: {error}</div>
                    </div>
                ) : (
                    <ManageTable
                        data={snackData}
                ...
                {user?.roles?.includes('branchmanager') && user?.branch && (
                    <BranchButton isLoading={branchLoading && !userBranch && !user?.branch?.name} branchName={userBranch?.name || user?.branch?.name || 'Unknown Branch'} />
                )}
