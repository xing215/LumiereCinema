            ...
            if (response && userContext && navigate && showError && !isWhitelisted) {
                if (response.status === 401) {
                    ...
                    if (userContext.isAuthenticated) {
                        ...
                        showError(403, 'Access denied. You do not have permission to access this resource.');
                    } else {
...
export default {
    ...
    get AUTH_ERROR_WHITELIST_ALIASES() {
        return [...AUTH_ERROR_WHITELIST_ALIASES];
    }, // Export aliases for external configuration
    get AUTH_ERROR_WHITELIST() {
        return getWhitelistPaths();
