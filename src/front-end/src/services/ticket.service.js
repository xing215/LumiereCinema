...
export const ticketService = {
  ...
  cleanupExpiredHolds: async (authToken) => {
    const response = await axios.post(getApiUrl('cleanupExpiredHolds'), {}, {
      headers: { Authorization: `Bearer ${authToken}` }
    });
    return response.data;
  },
  ...
  cleanupCache: async (authToken) => {
    const response = await axios.post(getApiUrl('cleanupCache'), {}, {
      headers: { Authorization: `Bearer ${authToken}` }
    });
    return response.data;
  },
  ...
  checkinTicket: async (ticketCode, authToken) => {
    const response = await axios.patch(getApiUrlWithParams('movieTicketDetails', { ticketCode }), 
      { status: 'CheckedIn' }, 
      {
        headers: { Authorization: `Bearer ${authToken}` }
      }
