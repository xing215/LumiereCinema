...
const BranchButton = ({ isLoading, branchName, clickable = false }) => {
    ...
    const displayName = isLoading ? 'LOADING...' : `  ${branchName?.toUpperCase() || ''}`;
    ...
    return (
        <button className={`absolute bottom-5 left-1/2 h-9 w-96 -translate-x-1/2 transform ${clickable ? 'hover:cursor-pointer' : ''}`}>
            ...
            <div className="font-unbounded absolute top-1/2 left-1/2 -translate-1/2 transform justify-start text-center text-base font-bold text-nowrap text-white">{displayName}</div>
        </button>
    );
};
