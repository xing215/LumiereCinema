...
const QuickActions = ({ suggestions = [], quickActions = [], onAction }) => {
    ...
    const allActions = [...(Array.isArray(quickActions) ? quickActions.map((qa) => ({ ...qa, type: 'action' })) : [])];
            ...
            <div className="flex flex-wrap gap-2">
                {allActions.map((action, index) => (
                    <button
                        ...
                        className={`transform rounded-lg px-3 py-2 text-xs font-medium break-words shadow-sm transition-all hover:scale-105 ${
