...
const MessageRenderer = ({ message, onQuickAction, sessionId }) => {
    ...
    if (!message.botData || !message.botData.type) {
        return (
            <div className="inline-block max-w-[250px] rounded-xl bg-sky-400 px-3 py-2 text-sm break-words text-white shadow-[inset_0px_0px_50px_3px_rgba(155,47,255,1.00)] sm:max-w-xs lg:max-w-md">
                {message.message}
            </div>
        );
    }
        ...
        case 'movie_details':
            return (
                <div className="space-y-3">
                    ...
                    {/* Movie Card */}
                    <MovieCard movie={data} onAction={handleQuickAction} quickActions={quick_actions} onMovieInteraction={reportMovieInteraction} />
                </div>
            );
        case 'movie_list':
            return (
        ...
        case 'movie_list_for_schedule':
            return (
                <div className="space-y-3">
                    ...
                    {/* Quick Actions */}
                    {(suggestions || []).length > 0 && <QuickActions suggestions={suggestions} onAction={handleQuickAction} />}
                </div>
            );
        case 'schedule_list':
            return (
                <div className="space-y-3">
                    ...
                    {/* Schedule List */}
                    <ScheduleList scheduleData={data} onAction={handleQuickAction} suggestions={suggestions} onScheduleInteraction={reportScheduleInteraction} />
                </div>
            );
        case 'follow_up_question':
            return (
                <div className="space-y-3">
                    ...
                    {/* Quick Actions */}
                    <QuickActions suggestions={suggestions} quickActions={message.botData.quick_actions} onAction={handleQuickAction} />
                </div>
            );
        case 'non_movie_related':
            return (
                <div className="space-y-3">
                    ...
                    {suggestions && <QuickActions suggestions={suggestions} onAction={handleQuickAction} />}
                </div>
            );
        case 'schedule_conversation':
            return (
