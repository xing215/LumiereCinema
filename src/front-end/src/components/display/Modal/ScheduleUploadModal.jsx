...
const ScheduleUploadModal = ({ 
            ...
            const processed = uploadedData.map((row, index) => {
                ...
                if (row.movieName && row.movieName.trim() !== '') {
                    foundMovie = movies.find(movie => 
                        movie.title.toLowerCase() === row.movieName.toLowerCase()
                    );
                ...
                if (!row?.screenName) {
                    ...
                    const screenExists = screens.some(screen => 
                        screen.screenName.toString() === row.screenName.toString()
                    );
                    if (!screenExists) {
                ...
                if (row.startTime && calculatedEndTime && row.date && row.screenName && errors.length === 0) {
                    ...
                    for (const existingSchedule of existingSchedules) {
                        ...
                        if (existingScreenName === row.screenName.toString() && 
                            checkTimeOverlap(row.startTime, calculatedEndTime, existingStartTime, existingEndTime, row.date, existingDate)) {
                    ...
                    for (let i = 0; i < uploadedData.length; i++) {
                        ...
                        if (otherRow.screenName?.toString() === row.screenName.toString() && 
                            otherRow.date === row.date &&
                            otherRow.startTime && otherRow.startTime.trim() !== '') {
                            
                            let otherEndTime = otherRow.endTime;
                            // Calculate other row's end time if needed
                            const otherMovie = movies.find(movie => 
                                movie.title.toLowerCase() === otherRow.movieName?.toLowerCase()
                            );
    ...
    return (
        <div 
            className="fixed inset-0 z-100 flex items-center justify-center w-full h-full bg-slate-900/50 backdrop-blur-sm"
        ...
        >
            ...
            <div className="relative w-[90%] max-w-7xl h-[80%] bg-white rounded-xl shadow-2xl flex flex-col">
                {/* Header */}
                <div className="flex items-center justify-between pt-6 px-6">
                    <h2 className="text-2xl font-bold font-['Unbounded'] text-gray-800">
                        Import Schedule Data
                    </h2>
                    <button
                        onClick={handleCancel}
                        disabled={isLoading}
                        className={`text-gray-500 hover:text-gray-700 text-2xl font-bold ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                        ×
                    </button>
                </div>
                ...
                {/* Content */}
                <div className="flex-1 overflow-hidden flex flex-col gap-2 justify-start">
                    <div className="">
                        <p className="pl-6 text-sm text-black mt-1 font-libre-franklin">
                            Total rows: {processedData.length} | 
                            Valid: {processedData.filter(row => row.isValid).length} | 
                            Invalid: {processedData.filter(row => !row.isValid).length} |
                            To Import: {processedData.filter(row => !tickedRows.has(row.id) && row.isValid).length}
                        </p>
                    </div>

                </div>
<div className="flex-1 absolute -top-10 w-full h-full overflow-hidden">
                        <ManageTable
                            data={getTableData()}
                ...
                <div className="flex items-center justify-center gap-4 p-6 border-t border-gray-200">
                    ...
                    <ConfirmButton 
                        ...
                        disabled={isLoading || processedData.filter(row => !tickedRows.has(row.id) && row.isValid).length === 0}
                    />
                </div>
            </div>
        </div>
