...
import { useGetScreens, useCreateScreen, useUpdateScreen, useRemoveScreen, useGetScreenSeats, useBulkCreateSeats } from '@hooks/useBranch';
...
export const useScreenManagement = () => {
    ...
    const filterScreens = useCallback(
        (screensList) => {
    ...
    const handleNewScreenFieldChange = useCallback(
        (columnIndex, value) => {
    ...
    const validateScreenName = useCallback(
        (screenName) => {
            ...
            const existingScreen = screensArray.find((screen) => screen.screenName?.toLowerCase().trim() === screenName.toLowerCase().trim());
            ...
            if (screen && fieldName && branchId) {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        showSuccess('Screen Updated', `Screen "${screenName}" has been updated successfully.`);
                    } else {
                    ...
                    showError('Error Occurred', 'An unexpected error occurred while updating the screen. Please try again.');
                } finally {
                    setIsUpdating(false);
                    setEditingCell(null);
                }
    ...
    const handleConfirmAddScreen = useCallback(async () => {
        ...
        try {
                ...
                if (newScreenId) {
                    ...
                    try {
                        ...
                        if (seatsResult.success || seatsResult.seats) {
                            ...
                            showSuccess('Screen Created Successfully!', `Screen "${screenToAdd.screenName}" has been created with ${seatsData.length} seats.`);
                        } else {
                            ...
                            showError('Screen Creation Failed', `Failed to create seats for the screen. Screen creation has been cancelled.`);
                    ...
                    showError('Screen Creation Failed', `Screen creation failed - no valid screen ID returned. Please try again.`);
            ...
            // Show generic error notification for unexpected errors
            showError('Error Occurred', 'An unexpected error occurred while creating the screen. Please try again.');
        } finally {
            setIsUpdating(false);
        }
        ...
        async (rowIndex, newIsHidden) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    showSuccess('Screen Status Updated', `Screen "${targetScreen.screenName || targetScreen.name}" has been ${statusText} successfully.`);
                } else {
                    // Parse error message for user-friendly display
                ...
                showError('Error Occurred', 'An unexpected error occurred while updating the screen status. Please try again.');
            } finally {
    ...
    const handleDeleteConfirm = useCallback(async () => {
        ...
        try {
            ...
            if (deletedScreenNames.length > 0 && failedDeletes === 0) {
                ...
                const message =
                    deletedScreenNames.length === 1 ? `Screen "${deletedScreenNames[0]}" has been deleted successfully.` : `${deletedScreenNames.length} screens have been deleted successfully.`;
            ...
            // Show generic error notification for unexpected errors
            showError('Error Occurred', 'An unexpected error occurred while deleting screens. Please try again.');
        }
    ...
    const getScreenByIndex = useCallback(
        (rowIndex) => {
            const filteredScreens = filterScreens(screens || []);
            const screensArray = Array.isArray(filteredScreens) ? filteredScreens : [];
