...
export const useMovieManagement = () => {
    ...
    const validateMoviesBatch = useCallback(
        (moviesData, existingMovies = []) => {
            const allErrors = [];
            const validMovies = [];
    ...
    // Inline editing hook
    const { editingCell, startEdit, saveEdit, cancelEdit, isUpdating } = useInlineEdit(updateMovie, getMovies, movies, setMovies);
    ...
    const handleNewMovieFieldChange = useCallback(
        (columnIndex, value) => {
            const fieldName = columnFieldMapping[columnIndex];
            if (fieldName) {
    ...
    const handleDeleteConfirm = useCallback(async () => {
        ...
        try {
            ...
            if (reviewMovieIndices.length > 0) {
                const updatedReviewMovies = reviewMovies.filter((_, index) => !reviewMovieIndices.includes(index));
                setReviewMovies(updatedReviewMovies);
        ...
        async (parsedData) => {
            ...
            try {
                ...
                if (validationResult.hasErrors) {
                    const confirmResult = await showUploadResults(validationResult.validMovies.length, validationResult.errors.length, validationResult.errors);
