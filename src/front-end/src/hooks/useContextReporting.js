...
export const useContextReporting = (sessionId) => {
     ...
     */
    const reportMovieInteraction = useCallback(
        async (movie, interactionType) => {
            if (!sessionId || !movie) return;

            try {
                await contextReportingService.reportMovieInteraction(movie, interactionType, sessionId, token);
            } catch (error) {
     ...
     * @param {string} interactionType - Type of interaction
     */
    const reportScheduleInteraction = useCallback(
        async (schedule, interactionType) => {
            if (!sessionId || !schedule) return;

            try {
                await contextReportingService.reportScheduleInteraction(schedule, interactionType, sessionId, token);
            } catch (error) {
     ...
     */
    const reportChatInteraction = useCallback(
        async (actionData) => {
            if (!sessionId || !actionData) return;

            try {
                await contextReportingService.reportChatInteraction(actionData, sessionId, token);
            } catch (error) {
