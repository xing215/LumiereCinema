...
export const useEnhancedMovieSearch = () => {
        ...
        async (keyword, page = 1, limit = 10) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    const formattedResults = result.results.map((movie) => movieSearchService.formatMovieResult(movie));
    ...
    const debouncedSearch = useCallback(
        (keyword, page = 1, limit = 10, delay = 500) => {
            if (searchTimeoutRef.current) {
                clearTimeout(searchTimeoutRef.current);
            }
...
export const useMovieAutocomplete = (options = {}) => {
    const { minLength = 2, debounceDelay = 300, maxSuggestions = 5 } = options;
...
export const useSearchHistory = (maxHistorySize = 10) => {
    ...
    const addToHistory = useCallback(
        (keyword) => {
            ...
            setHistory((prevHistory) => {
                ...
                const filtered = prevHistory.filter((item) => item.keyword.toLowerCase() !== cleanKeyword.toLowerCase());
    ...
    const removeFromHistory = useCallback((keyword) => {
        setHistory((prevHistory) => {
            const filtered = prevHistory.filter((item) => item.keyword.toLowerCase() !== keyword.toLowerCase());
