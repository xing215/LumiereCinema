...
import { useGetPromotions, useAddPromotion, useUpdatePromotion, useRemovePromotion } from '@hooks/useAdmin';
...
export const usePromotionManagement = () => {
    ...
    const handleNewPromotionFieldChange = useCallback(
        (columnIndex, value) => {
    ...
    const validatePromotionCode = useCallback(
        (promotionCode) => {
            ...
            const isDuplicate = existingPromotions.some((promotion) => promotion.promotionCode.toUpperCase() === promotionCode.toUpperCase());
    ...
    const onStatusChange = useCallback(
        async (rowIndex, newIsHidden) => {
            ...
            try {
                ...
                const result = await updatePromotion(promotion.promotionCode, { isActive: newIsActive });
    ...
    const handleDeleteConfirm = useCallback(async () => {
        ...
        const filteredPromotions = filterPromotions(promotions);
        const promotionsToDelete = Array.from(tickedPromotions)
            .map((index) => {
        ...
        try {
            const deletePromises = promotionsToDelete.map((promotion) => removePromotion(promotion.promotionCode));
    ...
    const getPromotionByIndex = useCallback(
        (rowIndex) => {
            if (isAddingPromotion && rowIndex === 0) {
