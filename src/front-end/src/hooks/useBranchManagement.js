...
export const useBranchManagement = () => {
  ...
  const filterBranches = useCallback((branchesList) => {
  ...
  const handleNewBranchFieldChange = useCallback((columnIndex, value) => {
  ...
  const checkBranchSchedules = useCallback(async (branchId) => {
    try {
      ...
      if (schedules && Array.isArray(schedules) && schedules.length > 0) {
        ...
        console.log('📋 [checkBranchSchedules] Active schedules details:', activeSchedules.map(s => ({
          _id: s._id,
  ...
  const validateBranchName = useCallback((branchName) => {
    ...
    const existingBranch = filteredBranches.find(branch => 
      branch.name?.toLowerCase().trim() === branchName.toLowerCase().trim()
    );
  ...
  const handleSaveEdit = useCallback(async (rowIndex, columnIndex, newValue) => {
    ...
    try {
      ...
      showSuccess(
        'Branch Updated',
        `Branch "${branch.name}" has been updated successfully.`
      );
    } catch (error) {
  ...
  const handleConfirmAddBranch = useCallback(async () => {
    ...
    try {
      ...
      showSuccess(
        ...
        `Branch "${branchData.name}" has been created successfully.`
      );
  ...
  const onStatusChange = useCallback(async (rowIndex, newIsHiddenOrIsActive) => {
    ...
    if (!newIsActive && branch.isActive) {
      try {
        ...
        showError(
          'Cannot Verify Branch Status',
          'Unable to check branch schedules. Please try again or contact administrator.'
        );
        return;
      }
    }
    ...
    try {
      ...
      showSuccess(
        ...
        `Branch "${branch.name}" has been ${statusText} successfully.`
      );
    } catch (error) {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      const filteredBranches = filterBranches(branches || []);
      const branchesToDelete = Array.from(tickedBranches).map(index => {
      ...
      if (deletedBranchNames.length > 0 && failedDeletes === 0) {
        ...
        const message = deletedBranchNames.length === 1
          ? `Branch "${deletedBranchNames[0]}" has been deleted successfully.`
          : `${deletedBranchNames.length} branches have been deleted successfully.`;
      ...
      showError(
        'Error Occurred',
        'An unexpected error occurred while deleting branches. Please try again.'
      );
    }
  ...
  const getBranchByIndex = useCallback((rowIndex) => {
    const adjustedRowIndex = isAddingBranch ? rowIndex - 1 : rowIndex;
    const filteredBranches = filterBranches(branches || []);
